...
export async function getProductosCategoria(req:Request, res:Response): Promise<Response> {

    const { cat } = req.params;
    console.log("Obteniendo productos por categoria: " + cat)

    const productosCat = await Producto.find({categoria:cat}); 
    console.log("Res cat: " +productosCat)
    return res.json(productosCat)
}
